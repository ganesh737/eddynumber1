...
export const FRAMES_TOOL_SCHEMAS: AgentToolSchema[] = [
  {
    ...
    description: [
      ...
      'Provide exact coordinates or count; with neither, samples evenly (default 4, max 8). Multi-frame results are one labeled contact sheet when possible.',
    ].join(' '),
    input_schema: {
      type: 'object',
      properties: {
        frames: { type: 'array', items: { type: 'number' }, maxItems: 8, description: 'Absolute timeline frame numbers to render.' },
        seconds: { type: 'array', items: { type: 'number' }, maxItems: 8, description: 'Absolute timeline seconds (converted by timeline fps).' },
        count: { type: 'number', minimum: 1, maximum: 8, description: 'Even midpoints across the timeline/range (default 4, max 8).' },
  ...
  },
  {
    ...
    description: [
      ...
      'Default broad scan = 6 source-window midpoints, max 8. Reuse the contact sheet instead of repeatedly sampling the same range. Finalized uploaded video prefers ffmpeg; blob/image/MG paths use browser or Remotion rendering.',
    ].join(' '),
    input_schema: {
      type: 'object',
      properties: {
        ...
        sourceTimesMs: {
          ...
          items: { type: 'number' },
          maxItems: 8,
          description: 'Millisecond offsets in source-media time. Accepts 1–8 values.',
        },
        frames: { type: 'array', items: { type: 'number' }, maxItems: 8, description: 'Source-media frame numbers (project/source timebase).' },
        seconds: { type: 'array', items: { type: 'number' }, maxItems: 8, description: 'Source-media seconds.' },
        count: { type: 'number', minimum: 1, maximum: 8, description: 'Even source-window midpoints (default 6 for video, max 8).' },
